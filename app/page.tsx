  ...
  return (
    <main>
      ...
      <section className="hero" id="top">
        ...
        <div className="orb orb-one" data-parallax="-0.12" aria-hidden="true" /><div className="orb orb-two" data-parallax="0.16" aria-hidden="true" />
        <div className="hero-copy reveal">
          <div className="eyebrow"><span className="pulse" /> Инфраструктура в России <b>RU / 01</b></div>
          <h1>ИИ для бизнеса.<br /><span>На ваших условиях.</span><i aria-hidden="true">↗</i></h1>
          ...
          <div className="hero-note"><span>01</span><p>От идеи и архитектуры<br />до промышленного запуска</p></div>
        </div>
        <div className="hero-panel" data-parallax="0.035">
          <div className="panel-head"><span>airent24 / control plane</span><span className="live"><i /> система доступна</span></div>
          ...
          <div className="signal-panel" aria-hidden="true"><span>LOAD</span><div>{[34, 58, 42, 78, 64, 88, 72, 96, 81, 100].map((height, index) => <i key={index} style={{ height: `${height}%` }} />)}</div><b>STABLE</b></div>
      ...
      <section className="section motion-reveal" id="solutions">
        ...
        <div className="service-grid">{services.map((item) => <article className="service-card" key={item.number}><span>{item.number}</span><div className="service-mark" aria-hidden="true"><i /><i /><i /></div><h3>{item.title}</h3><p>{item.text}</p><div className="card-arrow">↗</div></article>)}</div>
      </section>

      <section className="placement motion-reveal" id="placement">
      ...
      <section className="section dark-section motion-reveal">
      ...
      <section className="section principles motion-reveal" id="approach">
        ...
        <div className="principle-grid">{principles.map((item, index) => <article key={item.title} data-index={`0${index + 1}`}><span>0{index + 1}</span><h3>{item.title}</h3><p>{item.text}</p></article>)}</div>
      </section>

      <section className="process section motion-reveal"><span className="kicker">Как мы работаем</span><div className="process-line">{["Аудит задачи", "Архитектура", "Пилот", "Интеграция", "Сопровождение"].map((item, i) => <div key={item}><span>{i + 1}</span><p>{item}</p></div>)}</div></section>

      <section className="section faq-section motion-reveal" id="faq">
      ...
      <section className="request motion-reveal" id="request">
